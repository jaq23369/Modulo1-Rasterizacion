    ...
    def glRender(self):
        for model in self.models:
            ...
            for i in range(0, len(model.vertices), 3):
                ...
                # Inicializar las normales con valores por defecto
                nx = ny = nz = 0.0
                # Inicializar las UVs con valores por defecto
                u = v = 0.0

                # Procesar normales correspondientes si existen
                if hasattr(model, 'normals') and len(model.normals) >= i + 3:
                    nx = model.normals[i]
                    ny = model.normals[i + 1]
                    nz = model.normals[i + 2]
                
                # Procesar coordenadas UV si existen
                if hasattr(model, 'texcoords') and len(model.texcoords) >= (i//3) * 2 + 1:
                    u = model.texcoords[(i//3) * 2]
                    v = model.texcoords[(i//3) * 2 + 1]
                ...
                if self.activeVertexShader:
                    transformed = self.activeVertexShader(vertex = [x,y,z],
													    modelMatrix = self.activeModelMatrix)
                    x, y, z = transformed
                    
                # Agregar vértice Y su normal al buffer
                vertexBuffer.extend([x, y, z, nx, ny, nz, u, v])

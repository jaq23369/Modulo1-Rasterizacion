...
class OBJLoader:
    def __init__(self, filename):
        ...
        self.normals = []
        # Lista para almacenar los índices de normales por cara
        self.face_normals = []
        # Lista para coordenadas UV
        self.texcoords = []
        # Lista para índices de UV por cara
        self.face_texcoords = []
        ...
        try:
            with open(filename, 'r') as file:
                ...
                for line in file:
                    ...
                    elif parts[0] == 'vn':  # Procesar normales
                        nx = float(parts[1])
                        ny = float(parts[2])
                        nz = float(parts[3])
                        self.normals.append([nx, ny, nz])
                    
                    elif parts[0] == 'vt':  # Procesar coordenadas de textura
                        u = float(parts[1])
                    ...
                    # Si en dado caso no es un vértice, revisa si es una cara
                        ...
                        if len(face_vertices) >= 3:
                            for i in range(1, len(face_vertices) - 1):
                                ...
                                if face_normal_indices:  # Si hay normales
                                    normal_triangle = [
    ...
    def getNormals(self):
        normal_list = []
        if not self.face_normals:  # Si no hay normales definidas
            return None
